...
The `next_extra_block_relay` time is also defined. Periodically (on average every 5 minutes) the node connects to a peer (using regular outbound selection methodology from `addrman`) and stays connected long enough to sync headers, but not much longer. Then the peer is disconnected, if the node hasn't learned anything new. +
...
The fixed seeds are retrieved from `CChainParams::FixedSeeds()`.
